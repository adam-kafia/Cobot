...
if(isset($_POST["add_to_cart"]))
{
	if(isset($_SESSION["shopping_cart"]))
	{
		...
		$sql="insert into panier (qte,nom,prix) values (:qte,:nom,:prix)";
		$db = config::getConnexion();
		try{
        $req=$db->prepare($sql);
		$nom = $_POST["hidden_name"];
	...
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values)
		{
			if($values["item_id"] == $_GET["id"])
			{
				...
				echo '<script>window.location="shop-list.php"</script>';
			}
		}
	}
}
	...
	<body class="preload">
		...
		<header>
			...
			<nav class="navbar-desktop">
				...
				<div class="right">
					...
					<br>
											<a> <?php
												$query = "DELETE * FROM panier where id = :item_id";
												?> <a href="shop-list.php?action=delete&id=<?php echo $values["item_id"]; ?>" 
												>
												<span class="lnr lnr-cross-circle"></span>
											</a></a>					    
											<a href="#" class="image-holder">
												<span class="product-name"><?php echo $values["item_name"]; ?></span>
											</a>
											<span class="quantity"> 
												<span class="woocommerce-Price-amount amount">
													<br>
													<span class="woocommerce-Price-currencySymbol"><?php echo $values["item_price"]; ?> DT</span>
												</br>
												</span>
												<br>
					...
					?>
									<p class="woocommerce-mini-cart__total total">
										...
										<span class="woocommerce-Price-amount amount color-cdaa7c">
											<span class="woocommerce-Price-currencySymbol"><?php echo number_format($total, 2); ?></span>DT
										</span>
									</p>
									<?php
					}
		...
		<main>
			...
			<section class="section-primary pt-150 pb-113 shop-list">
				...
				?>
				<div class="col-md-4">
				<form method="post" action="shop-list.php?action=add&id=<?php echo $row["id"]; ?>">
